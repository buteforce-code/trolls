...
3. **Writer** — digest → 1,400–2,000-word MDX, anti-fluff rules, India-first ICP, SEO keyword placement. A **length gate** in the orchestrator (`MIN_BODY_WORDS = 1100`, frontmatter excluded) enforces the floor, then fails the topic to human review rather than publishing a thin post. The prompt alone was never enough: after the 2026-06-29 Gemini→gpt-4o switch, output silently halved and five ~550-word posts shipped before anyone noticed.

   When the draft is short the gate **expands it section by section** rather than re-asking for the whole post. gpt-4o settles around 1,000 output tokens whatever the prompt says (`max_tokens` is 4096, so this is not truncation), and feeding it the short draft anchors the retry to that length — on 2026-08-02 a re-ask went 634 → 764 words against the 1,100 floor and the run failed. So the gate splits the body at `##`, rewrites the thinnest section at ~320 words per call (≤8 calls, ceiling 420 words each), and reassembles until the post clears `_LENGTH_TARGET_WORDS = 1450` — above the floor, because the humaniser and the GEO repair pass both trim. Replies are re-headed with the original heading (question H2s and "not a fit if…" are GEO surfaces) and a reply 3× the size asked for is discarded as a whole-post re-emission.
